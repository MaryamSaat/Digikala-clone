...
const SubCategoryButton = memo(({ title, isActive, onClick }: { title: string; isActive: boolean; onClick: () => void }) => (
  <button onClick={onClick} className="w-full px-4 py-3 border-b border-gray-200 hover:bg-gray-50 transition-colors text-right flex items-center justify-between">
    <span className="text-gray-900 font-['IRANYekanWeb'] text-[12px]">{title}</span>
    ...
    <div className="fixed inset-0 bg-white z-40 flex">
        ...
        <div className="flex flex-col">
          {categories.map((category) => (
            ...
            >
              ...
              <span className={`font-['IRANYekanWeb'] text-[10px] transition-colors ${activeCategoryId === category.id ? "text-red-500" : "text-[#3f4064] group-hover:text-red-500"}`}>{category["main-title"]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="w-4/5 bg-white h-full overflow-y-auto">
        {activeMobileCategory && (
          <div className="flex flex-col">
            {/* لینک همه محصولات */}
            <a href={activeMobileCategory.url} className="px-4 py-3 text-[11px] leading-[2.17] text-[#008eb2] font-['IRANYekanWeb'] hover:text-[#008eb2] transition-colors">
              {activeMobileCategory.generalLink} &gt;
            </a>

            {/* عنوان اصلی */}
            <div className="px-4 py-3 border-b border-gray-200">
              <h2 className="text-[12px] font-['IRANYekanWeb'] leading-[2.15] text-gray-900">{activeMobileCategory["main-title"]}</h2>
            </div>
